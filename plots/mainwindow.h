...
public:
  ...
  ~MainWindow();
  // CLOOK graph
  void setup_CLOOK_Graph(QCustomPlot *customPlot);
  void setup_testing(QCustomPlot *customPlot);
  ...
  // CLOOK helper functions
  QVector<double> generateList(int size, int end_range);
  QVector<double> generatePredefinedList();
  void printVector(QVector<double> targetVector);
  int calc_CLOOK_movement(QVector<double> sortedVector, char direction, int initial_cylinder);
  int find_starting_pos(QVector<double> dVector, int initial_cylinder);
