  ...
  ui(new Ui::MainWindow)
{
  ...
  // 20: setup_CLOOK_Graph(ui->customPlot);
...
void MainWindow::setupDemo(int demoIndex)
{
  switch (demoIndex)
  {
    ...
    case 20: setup_CLOOK_Graph(ui->customPlot); break;
    case 21: setup_testing(ui->customPlot); break;
  }
...
/***********My random-experiemental testing******************/
void MainWindow::setup_testing(QCustomPlot *customPlot){
    demoName = "Experiemental Demo";
    // generate specific tick marks and labels for x-axis
    QVector<double> ticks = generatePredefinedList();
    QVector<QString> labels;
    for(int i = 0; i < ticks.size(); i++){ // allow matching ticks and labels
        labels << QString::number(ticks[i]);
    }
    // generate data onto (x,y) coordinate system
    int largest_queue_value = static_cast<int>(ticks.last());
    int temp = largest_queue_value;
    QVector<double> x(ticks.size()), y(ticks.size()); // initialize with entries
    for(int i=0; i<ticks.size(); ++i){
      x[i] = ticks[i]; // x-axis values
      y[i] = temp; // represents y-axis (values have no significant value)
      temp--;
    }
    // give the axes some labels:
    customPlot->xAxis->setVisible(true);
    customPlot->xAxis2->setVisible(true);
    customPlot->xAxis2->setLabel("Queue Cylinders");
    // set the ticker format
    QSharedPointer<QCPAxisTickerText> textTicker(new QCPAxisTickerText);
    textTicker->setTickStepStrategy(QCPAxisTicker::tssReadability);
    textTicker->addTicks(ticks, labels);

    // set axes ranges, so we see all data:
    customPlot->xAxis2->setRange(ticks[0], ticks[ticks.size()-1]);
    customPlot->xAxis2->setTicker(textTicker);
    customPlot->xAxis2->setTickLabelRotation(90);
    customPlot->xAxis->setRange(0, 199);
    customPlot->xAxis->setTicker(textTicker);
    customPlot->yAxis->setRange(0, 199);

    // recreate graph based on starting head position
    int initial_head = 2;
    QVector<double> newVector;
    for(int i = 0; i < ticks.size(); i++){
        int old_index = (initial_head + i) % ticks.size();
        newVector.push_back(ticks[old_index]);
    }
    // assign newVector representation onto graph
    for(int i=0; i<newVector.size(); ++i){
      x[i] = newVector[i]; // x-axis values
    }
    // draw graph and assign data to it:
    for(int i = 0; i < ticks.size()-1; i++){
        QCPItemLine *arrow = new QCPItemLine(customPlot);
//        customPlot->addGraph(customPlot->xAxis2, customPlot->yAxis);
        customPlot->addGraph();
        customPlot->graph()->setLineStyle((QCPGraph::lsLine));
        customPlot->graph()->addData(x[i], y[i]);           // pnt i
        customPlot->graph()->addData(x[i + 1], y[i + 1]);   // pnt (i + 1)
        // create arrow points at end of each line segment
        arrow->start->setCoords(x[i], y[i]);
        arrow->end->setCoords(x[i + 1], y[i + 1]);
...
/***********My test setup******************/
void MainWindow::setup_CLOOK_Graph(QCustomPlot *customPlot){
    demoName = "CLOOK Demo";
...
/*************[function definitions for CLOOK helper functions]**************/
using namespace std;
// generate random values
QVector<double> MainWindow::generateList(int size, int end_range){
    ...
    QSet<double> set; // ensures that no multiple copies are accepted
    int randomNum;
    while(set.size() < size){
        randomNum = rand() % end_range;
        if(set.contains(randomNum)){
            set << randomNum;
            dVector.push_back(randomNum);
        }
    }
    sort(dVector.begin(), dVector.end(), sortByAscending());
    return dVector;
}
// generate predefined values also sorts vector values in ascending order
QVector<double> MainWindow::generatePredefinedList(){
    //comparator for sorting
    struct sortByAscending {
      bool operator() (int i,int j){
          return (i<j);
...
//    whole thing is from 0-199, start at 53
    QVector<double> queue_request = {98, 183, 37, 122, 14, 124, 65, 67};
    sort(queue_request.begin(), queue_request.end(), sortByAscending());
//    printVector(queue_request);
    return queue_request;
}
void MainWindow::printVector(QVector<double> targetVector){
    printf("my targetVector contains: %d elements:\n", targetVector.size());
    for(int i = 0; i < targetVector.size(); i++){
        printf("%f ", targetVector[i]);
    }
    printf("\n");
}
// CLOOK implementation of total head movement
int MainWindow::calc_CLOOK_movement(QVector<double> sortedVector, char direction, int initial_cylinder){
    int start_pos = find_starting_pos(sortedVector, initial_cylinder);
    printf("starting position: %u\n", start_pos);
    int head_movement_sum = abs(sortedVector[start_pos] - initial_cylinder);
    printf("initial head movement: %d\n", head_movement_sum);
    bool wrap_around = false; // indicates if we have jumped to the other side
    int current_index = start_pos;
    int next_index = current_index + 1;
    // right direction
    if(direction == 'r' || direction == 'R'){
        while(!wrap_around){
            printf("next index: %d\n", next_index);
            if(current_index == sortedVector.size() - 1){
                current_index = 0;
                next_index = current_index + 1;
                wrap_around = true; // don't calculate the distance of the circular-jump
            } else{
                head_movement_sum += abs(sortedVector[next_index] - sortedVector[current_index]);
                current_index++;
                next_index = current_index + 1;
                printf("head movement sum: %d\n", head_movement_sum);
            }
        }
        while(next_index < start_pos){
            printf("next index: %d\n", next_index);
            head_movement_sum += abs(sortedVector[next_index] - sortedVector[current_index]);
            current_index++;
            next_index++;
        ...
        return head_movement_sum;
    }
    // implement left side here ...
    return 0;
}
// finds index closest to starting position
int MainWindow::find_starting_pos(QVector<double> dVector, int initial_cylinder){
    int distance = abs(initial_cylinder - static_cast<int>(dVector[0]));
    int index = 0;
    // find min distance
    for(int i = 1; i < dVector.size(); i++){
        int temp = abs(initial_cylinder - static_cast<int>(dVector[0]));
        if(distance > temp){
            distance = temp;
            index = i;
        }
